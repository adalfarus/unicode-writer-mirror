...
# Shortcut keys
shortcut_keys = {keyboard.Key.alt_l, keyboard.Key.caps_lock}

def on_press(key):
    if key in shortcut_keys:
        current_keys.add(key)
    if current_keys == shortcut_keys:  # check if all shortcut keys are pressed
        try:
            char = key.char.lower()
...
def on_release(key):
    ...
    if key in shortcut_keys:
        ...
        if not current_keys and number_string:  # when all shortcut keys are released
            try:
